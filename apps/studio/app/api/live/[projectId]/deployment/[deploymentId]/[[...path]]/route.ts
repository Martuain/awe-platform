...
async function handle(
  request: NextRequest,
  context: { params: Promise<{ projectId: string; deploymentId: string; path?: string[] }> },
) {
  ...
  const target = `${apiBase}/api/v1/website-preview/live-proxy/${projectId}/${suffix}?${query.toString()}`;
  try {
    ...
    return NextResponse.json({ error: "Deployment runtime is unreachable.", detail: String(error) }, { status: 502 });
  }
}
