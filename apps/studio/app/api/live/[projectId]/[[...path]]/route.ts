...
async function handle(
  ...
  const target = `${apiBase}/api/v1/website-preview/live-proxy/${projectId}/${suffix}${request.nextUrl.search}`;
  try {
    ...
    return NextResponse.json({ error: "Live deployment is unreachable.", detail: String(error) }, { status: 502 });
  }
}
