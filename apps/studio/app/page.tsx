...
type Stage = "discovery" | "strategy" | "design" | "specification" | "generation" | "build" | "preview" | "deployment";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    api<Project>(`/api/v1/projects/${saved}`)
      .then(async (p) => {
        setProject(p);
        try {
          const w = await api<Workspace>(`/api/v1/projects/${p.id}/workspace`);
          setWorkspace(w);
                    ...
                    try {
                      ...
                      if (w.status === "approved") {
                        try {
                          ...
                          setGeneration(g);
                          try {
                            const b = await api<WebsiteBuildPlan>(`/api/v1/website-build/plan?project_id=${p.id}`, { method: "POST" });
                            setBuildPlan(b);
                            if (b.status !== "planned") { setStage("build"); return; }
                            try {
                              const v = await api<WebsiteValidation>(`/api/v1/website-validation/validate?project_id=${p.id}`, { method: "POST" });
                              setValidation(v);
                              setStage("preview");
                            } catch { setStage("build"); }
                          } catch { setStage("build"); }
  ...
  async function generateWebsite() {
    ...
    try {
      ...
      setGeneration(g); setStage("build");
  ...
  async function executeBuild() {
    ...
    try {
      ...
      setStage("preview");
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to execute website build"); }
    finally { setBusy(false); }
  }
  ...
  return (
    <main className="shell">
      ...
      <nav className="pipeline" aria-label="AWE capability pipeline">
        {[ ["discovery", "01", "Discovery"], ["strategy", "02", "Strategy"], ["design", "03", "Design"], ["specification", "04", "Website Spec"], ["generation", "05", "Generate"], ["build", "06", "Build"], ["preview", "07", "Preview"] ].map(([key, number, label]) => (
          <button key={key} className={stage === key ? "active" : ""} onClick={() => setStage(key as Stage)} disabled={!project || (key === "strategy" && context?.status !== "approved") || (key === "design" && strategy?.status !== "approved") || (key === "specification" && design?.status !== "approved") || (key === "generation" && specification?.status !== "approved") || (key === "build" && !generation) || (key === "preview" && !buildResult) || (key === "deployment" && !buildResult)}>
