...
type Stage = "discovery" | "strategy" | "design" | "specification" | "generation";
  ...
  return (
    <main className="shell">
      ...
      <nav className="pipeline" aria-label="AWE capability pipeline">
        {[ ["discovery", "01", "Discovery"], ["strategy", "02", "Strategy"], ["design", "03", "Design"], ["specification", "04", "Website Spec"], ["generation", "05", "Generate"] ].map(([key, number, label]) => (
          <button key={key} className={stage === key ? "active" : ""} onClick={() => setStage(key as Stage)} disabled={!project || (key === "strategy" && context?.status !== "approved") || (key === "design" && strategy?.status !== "approved") || (key === "specification" && design?.status !== "approved") || (key === "generation" && specification?.status !== "approved")}>
